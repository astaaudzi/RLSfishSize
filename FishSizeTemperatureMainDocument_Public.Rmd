...
This document is an electronic supplement to the NEE submission NATECOLEVOL-19108348-T. The document contains unpublished code and data. Please use it only for review purposes. 
...
### Extended Data Fig.3: Plot size-temperature distributions in space
...
### Extended Data Fig 7: testing if exclusion of juveniles might affect Fi1B results
...
### Extended Data Fig. 4: correlation to abundance skew
...
plot(bet1ci$skew, bet1ci$p50, pch=19, ylab = "Size - SST slope (b1) from spatial analyis (this study)", xlab = "Species abundance - SST skew from Waldock et al (2019)")
...
### Extended Data Table 2: long term SST trends
...
### Extended data Fig. 6: inside/outside MPA
...
##Summaries for spatial analyses
...
length(which(st$Time_betaloc50 > 0)) #54
#How many are increasing "significanlty" (80% PPD above 0)
length(which(st$Time_betaloc10 > 0)) #25
...
length(which(st$Time_betaloc50 < 0)) #95
#How many are decreasing "significanlty" (80% PPD below 0)
length(which(st$Time_betaloc90 < 0)) #39
#What is the average change per year for these "significant species"?
