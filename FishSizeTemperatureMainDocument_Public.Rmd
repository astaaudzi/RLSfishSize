...
main_data <- rls_st %>% select(Location, SurveyID, SiteCode, Diver, TAXONOMIC_NAME, SPECIES_EPITHET, GENUS, FAMILY, ORDER, SizeClass, day, month, year, geogroup, SiteLat, SiteLong, midpoint, LWa, LWb, GGD12, scaledGDD, meansst, MaxLenFB, MaxSizeObs, SST_1yago, SST_2yago, SST_3yago, SST_5yago)

### Extended Fig1: map data and explore 
...
```{r warning=FALSE, message=FALSE, warning=FALSE, echo=FALSE, eval = F}
...
#save(sizeplot, file = paste("C:/Users/astaa/Dropbox/asta/RLS/OptimalSize/output/newSpace/",SpeciesName,"sizeplot.RData",sep="")) 
 
#load(file = paste("C:/Users/astaa/Dropbox/asta/RLS/OptimalSize/output/",SpeciesName,"_plot.RData",sep=""))
...
### Extract postorior distribution quantiles from individual Bayesian analyses
...
save(bet1ci, file = "analysesOutputs/FinalSpaceBet1ci_335spp.RData")

write.csv(bet1ci, file = "FinalSpaceCI.csv")
...
#fit <- stan(file = 'fit_multi.stan', data = stan_dat,
#  iter = 3500, warmup = 1000, chains = 4, seed = 111, thin = 10)
#save(fit, file = "RDataOutputs/spacefit.RData")

load(file = "RDataOutputs/spacefit.RData")
...
#fit <- stan(file = 'fit_multi.stan', data = stan_dat,
#  iter = 3500, warmup = 1000, chains = 4, seed = 111, thin = 10)
#save(fit, file = "RDataOutputs/Lengthfit.RData")
#save(fit, file = "RDataOutputs/Lengthfit_exclusionTest.RData")

## Load the main fit data set
load(file = "RDataOutputs/Lengthfit.RData")
...
### Table 1: statistics of spatial trends
...
length(which(bet1ci$p50 < 0)) # 189 negative responses in total
length(which(bet1ci$p50 > 0)) # 146 positive

increasing <- bet1ci[which(bet1ci$p10 > 0),]
decreasing <- bet1ci[which(bet1ci$p90 < 0),]

tempSpace <- bet1ci %>% filter (midpoint <= 23)
tropSpace <- bet1ci %>% filter (midpoint > 23)
VerytropSpace <- bet1ci %>% filter (midpoint > 26.5)

SignTempSpp <- tempSpace$species[which(tempSpace$p10 > 0 | tempSpace$p90 <0)]
SignTropSpp <- tropSpace$species[which(tropSpace$p10 > 0 | tropSpace$p90 <0)]

length(which(tempSpace$p10 > 0)) #32(33)
length(which(tempSpace$p10 > 0))/length(SignTempSpp) #0.49
length(which(tempSpace$p90 < 0)) #35
length(which(tempSpace$p90 < 0)) / length(SignTempSpp) #0.51

length(which(tempSpace$p50 > 0)) #54(57)
length(which(tempSpace$p50 > 0))/length(tempSpace$species) #0.49
length(which(tempSpace$p50 < 0)) #57(59)
length(which(tempSpace$p50 < 0))/length(tempSpace$species) #0.51

length(which(tropSpace$p10 > 0)) #28(31)
length(which(tropSpace$p10 > 0))/length(SignTropSpp)
length(which(tropSpace$p90 < 0)) #61(62)
length(which(tropSpace$p90 < 0))/length(SignTropSpp)

length(which(tropSpace$p50 > 0)) #82(88)
length(which(tropSpace$p50 > 0))/length(tropSpace$species)
length(which(tropSpace$p50 < 0)) #126(130)
length(which(tropSpace$p50 < 0))/length(tropSpace$species)

length(which(VerytropSpace$p10 > 0)) #9
length(which(VerytropSpace$p90 < 0)) #23

length(which(VerytropSpace$p50 > 0)) #38
length(which(VerytropSpace$p50 < 0)) #62



#tropical
median(tropSpace$p50[which(tropSpace$p10 > 0)]) # 0.037 
median(tropSpace$p50[which(tropSpace$p90 < 0)]) # -0.040

#beta1 = -0.02, exp(-0.02) = 0.980, means a 2% 

median(tropSpace$p50[which(tropSpace$p50 > 0)]) # 0.022 
median(tropSpace$p50[which(tropSpace$p50 < 0)]) # -0.023


#temperate
median(tempSpace$p50[which(tempSpace$p10 > 0)]) # 0.054 
median(tempSpace$p50[which(tempSpace$p90 < 0)]) # -0.039

median(tempSpace$p50[which(tempSpace$p50 > 0)]) # 0.030 
median(tempSpace$p50[which(tempSpace$p50 < 0)]) # -0.031

#total
median(decreasing$p50) # -0.039
median(increasing$p50) # 0.042

median(tempChar$LWa, na.rm=T) ## 0.0177
median(tempChar$LWb, na.rm=T) ## 3.008

## if you are 15cm on average, 12 and 17 cm on edges
0.0177*12^3.008
0.0177*17^3.008
0.0177*15^3.008

(0.0177*15^3.008)/(0.0177*(15*1.04)^3.008)
(0.0177*15^3.008)/(0.0177*(15*0.96)^3.008)

tempChar <- fish_temp %>% group_by(TAXONOMIC_NAME) %>% summarise(tmin = min(meansst), tmax = max(meansst), range = tmax - tmin, medsize = median(SizeClass), meansize = mean(SizeClass), medwgt = median(weight, na.rm=T), midp = first(midpoint), LWa = first(LWa), LWb = first(LWb))

tempCharSignTemp <- tempChar %>% filter (TAXONOMIC_NAME %in% SignTempSpp)
median(tempCharSignTemp$range) #7.6
median(tempCharSignTemp$meansize) #15.9

tempCharSignTrop <- tempChar %>% filter (TAXONOMIC_NAME %in% SignTropSpp)
median(tempCharSignTrop$range) #7.6
median(tempCharSignTrop$meansize) #7.6


#for temperate species 
median(tempChar$range[which(tempChar$midp < 23)]) #mean temp range is 7.6C
median(tempChar$medsize[which(tempChar$midp < 23)]) #and median size is 15cm
mean(tempChar$meansize[which(tempChar$midp < 23)]) #and median size is 15cm
median(tempChar$medwgt[which(tempChar$midp < 23)]) #53g

#for temperate species 
median(tempChar$range[which(tempChar$midp > 23)]) #mean temp range is 7.9C
median(tempChar$medsize[which(tempChar$midp > 23)]) #and median size is 10cm
mean(tempChar$meansize[which(tempChar$midp > 23)]) #and median size is 10cm
median(tempChar$medwgt[which(tempChar$midp > 23)], na.rm=T) #25g

median(tempChar$LWa[which(tempChar$midp > 23)], na.rm=T) #0.019
median(tempChar$LWb[which(tempChar$midp > 23)], na.rm=T) #3.03

median(tempChar$LWa[which(tempChar$midp < 23)], na.rm=T) #0.015
median(tempChar$LWb[which(tempChar$midp < 23)], na.rm=T) #2.97

cor.test(bet1ci$p50, bet1ci$MaxLenObs)
...
AllParams <-AllParams %>% filter ((!locs %in% notwarming))


## explore temporals 
length(which(AllParams$betaloc50 > 0)) #56
length(which(AllParams$betaloc50 > 0))/length(AllParams$betaloc50) #0.36
length(which(AllParams$betaloc50 < 0))/length(AllParams$betaloc50)  #100
length(unique(AllParams$species)) #77

signiTemporal <- AllParams %>% filter (betaloc10 > 0 | betaloc90 < 0)
length(unique(signiTemporal$species))

length(which(AllParams$betaloc10 > 0))/length(signiTemporal$betazero50) #38%
length(which(AllParams$betaloc90 < 0))/length(signiTemporal$betazero50) #62%

(median(AllParams$betaloc50[which(AllParams$betaloc50 > 0)]))*100 #0.85
(median(AllParams$betaloc50[which(AllParams$betaloc50 < 0)]))*100 #0.53

(median(AllParams$betaloc50[which(AllParams$betaloc10 > 0)]))*100 #13.9%
(median(AllParams$betaloc50[which(AllParams$betaloc90 < 0)]))*100 #9.4%


```

### Add thermal skew (Waldock et al.)
...
bet1ci.t <- bet1ci %>% filter (skew > -10)

temp2 <- bet1ci.t %>% filter (p10 > 0 | p90 < 0)


cor.test(bet1ci$skew, bet1ci$p50)
plot(bet1ci$skew, bet1ci$p50, pch=19, ylab = "Size-temperature slope", xlab = "Abundance-temperature skew", main = "Correlation between size-temp and abund-temp trends")
abline(lm(p50 ~ skew, data = bet1ci))

cor.test(temp2$skew, temp2$p50)
plot(temp2$skew, temp2$p50, pch=19)


temp3 <- bet1ci %>% filter (range < 0.17)

cor.test(temp3$skew, temp3$p50)
plot(temp3$skew, temp3$p50, pch=19)
abline(lm(p50 ~ skew, data = temp3))

cor.test(bet1ci$Topt, bet1ci$p50)
plot(bet1ci$Topt, bet1ci$p50, pch=19)

#if we look only at those that have strong skew?
temp <- bet1ci %>% filter (skew < -1 | skew > 1)

cor.test(temp$skew, temp$p50)
plot(temp$skew, temp$p50, pch=19)

temp3 <- be

length(which(bet1ci$skew <0 & bet1ci$p50 <0)) #84 (129)
length(which(bet1ci$skew <0 & bet1ci$p50  > 0)) #55 (92)
length(which(bet1ci$skew > 0 & bet1ci$p50  > 0)) #15 (34)
length(which(bet1ci$skew > 0 & bet1ci$p50  < 0)) #18 (45)

```

### TEMPORAL ###

### Add mpa data to the main dataset 

```{r warning=FALSE, message=FALSE, warning=FALSE, echo=F, eval = F}

load(file = "inputs/VertData2018Cleaned_Mar.RData")
length(unique(VertData$SiteCode))  # this dataset has 2568 unique codes 

#load(file="inputs/sitesMPAstatus.RData") #dataset create above
#length(unique(sites_rls$SiteCode)) #the site summary data has 2577 codes

mpa_checked <- read.csv(file = "inputs/RLSMPAsitesWithMPA.csv") #checked MPA status dataset (from Graham)
mpa_checked$mpaFinal <- NA
mpa_checked$mpaFinal[which(mpa_checked$MPA == "Fishing")] <- 0 #133
mpa_checked$mpaFinal[which(mpa_checked$MPA == "No take")] <- 1 #159
mpa_checked$mpaFinal[which(mpa_checked$MPA == "Restricted take")] <- 0 #104

#add extra variable to the main rls_all data set to reflect site protection. This is only for Tasmanian sites
VertData$mpa <- mpa_checked$mpaFinal[match(VertData$`Site name`, mpa_checked$sitename)]

#Are there any sites that still have NA's for the MPA data? 
sites_mpa_status <- VertData %>% group_by(SiteCode) %>% summarise(mpa = first(mpa))
length(which(is.na(sites_mpa_status$mpa) == TRUE)) # yes, 716 out of 2568 sites

# I guess we have to assume that if there is no info for the site then it is not protected
#VertData$mpa[is.na(VertData$mpa)] <- 0

save(VertData, file = "inputs/VertDataWithMPA_marNew.RData")

```

### Generate and map long term location datafile for people to check 

```{r}
load(file = "inputs/VertDataWithMPA_marNew.RData")

VertData$Locs <- substr(VertData$Location, 1,6)

#Pool Bass Strait and Kent Group togehter as they are very close to each other. Also pool Port Sydney and Sydney
VertData$Loc=ifelse(VertData$Locs=="Bass S"|VertData$Locs=="Kent G", "BassSt", as.character(VertData$Locs)) 
badlocs <- c("Tasman", "North ")
...
#save(yearsPerLoc, file = "inputs/yearsPerLoc.RData")

VertData_temporal <- VertData %>% filter (Loc %in% longtermLoc)

save(VertData_temporal, file = "inputs/VertData_Temporal.RData")

yearsPerLoc$LocText <- as.character(yearsPerLoc$Loc)

load(file = "inputs/australia.RData")

ggmap(australia, extent = "panel", legend = "bottomright") +
  geom_point(aes(x = yearsPerLoc$longmean, y = yearsPerLoc$lattmean), data = yearsPerLoc, size = (yearsPerLoc$spp/40), color = "orange") +
#   geom_text(aes(label= yearsPerLoc$Loc), nudge_y = 0.001, size = 2) + 
  ggtitle("Long term locations") + xlab("Longitutde") + ylab("Latitude")

## write site data for Graham to check
#sites_mpa_status_longterm <- VertData %>% filter (Loc %in% longtermLoc) %>% group_by(SiteCode) %>% summarise(mpa = first(mpa), sitename = first(`Site name`), loc_short = first(Loc), loc_full = first(Location), latt = mean(SiteLat), long = mean(SiteLong)) %>% arrange(loc_full)
#write.csv(sites_mpa_status_longterm, file = "inputs/sitesWithMpa.csv")  

```

### Long term SST trends 
...
### Setup various dataframes

df - main dataframe of all species and 12 locations 
dfMPA9 - data with 9 locations only inside MPA
df9 - data with 9 locations only outside MPA
...
## Filter out rare species using same criteria
df_summary <- df_temp %>%
...
df_ok <- semi_join(df_temp, df_summary, 
...
This very simple analysis actually gives very similar result to the Bayesian slopes
...
#get mean size, temp and size quantiles for each cell
DataForIndReg <- fish_temp %>% group_by (TAXONOMIC_NAME, geogroup, year) %>% summarise (meansize = mean(SizeClass), mediansize = quantile(SizeClass, probs = 0.50), q75size = quantile(SizeClass, probs = 0.75), q25size = quantile(SizeClass, probs = 0.25), midpoint = first(midpoint), meantemp = first(meansst), temp1y = first(SST_1yago), temp2y = first(SST_2yago), temp3y = first(SST_3yago), temp5y = first(SST_5yago), gdd = first(scaledGDD), gdd1y = first(scGDD_1yago), gdd2y = first(scGDD_2yago), gdd3y = first(scGDD_3yago), gdd5y = first(scGDD_5yago), cumgdd2 = first(cumGDD_2y), cumgdd3 = first(cumGDD_3y), cumgdd5 = first(cumGDD_5y))

SppChar <- fish_temp %>% group_by(TAXONOMIC_NAME) %>% summarise(midp  = first(midpoint), lmaxFB = first(MaxLenFB), lmaxObs = first(MaxSizeObs))

Species <- unique(DataForIndReg$TAXONOMIC_NAME)


IndRegr = matrix(NA, nrow = length(Species), ncol = 6)

for (i in 1:length(Species)) {
  
onesp <- DataForIndReg %>% filter(TAXONOMIC_NAME == Species[i])
print(i)
print(Species[i])

mod1 = lm(meansize ~ cumgdd3, data = onesp)
mod2 = lm(q75size ~ cumgdd3, data = onesp)
mod3 = lm(q25size ~ cumgdd3, data = onesp)
#mod2 = lm(sizeZ ~ sstmean*relopt, data = onesp)

IndRegr[i,c(1:2)] <- mod1[[1]]
IndRegr[i,c(3:4)] <- mod2[[1]]
IndRegr[i,c(5:6)] <- mod3[[1]]
#IndRegr[i,3] <- as.character(Species[i])
}

colnames(IndRegr) = c("intMean", "slopeMean","intq75", "slopeq75","intq25", "slopeq25")

IndRegr <- as.data.frame(IndRegr)
IndRegr$TAXONOMIC_NAME <- Species
IndRegr$midpoint <- SppChar$midp[match(SppChar$TAXONOMIC_NAME, IndRegr$TAXONOMIC_NAME)]
IndRegr$LmaxFB <- SppChar$lmaxFB[match(SppChar$TAXONOMIC_NAME, IndRegr$TAXONOMIC_NAME)]
IndRegr$MaxSizeOb <- SppChar$lmaxObs[match(SppChar$TAXONOMIC_NAME, IndRegr$TAXONOMIC_NAME)]


IndRegr_annSST <- IndRegr #put this into a separate dataframe
save(IndRegr_annSST, file  = "RDataOutputs/IndRegrForMeanSST.RData")

IndRegr_gdd <- IndRegr #put this into a separate dataframe
save(IndRegr_gdd, file  = "RDataOutputs/IndRegrForGDD.RData")

IndRegr_temp2yago <- IndRegr #put this into a separate dataframe
save(IndRegr_temp2yago, file  = "RDataOutputs/IndRegrForTemp2yago.RData")
```

### Plot indiv regression results

```{r, echo=F}
##
load(file  = "RDataOutputs/IndRegrForMeanSST.RData")
load(file  = "RDataOutputs/IndRegrForGDD.RData")
load(file  = "RDataOutputs/IndRegrForTemp2yago.RData")

par(mfrow = c(3,1))

plot(IndRegr_annSST$midpoint, IndRegr_annSST$slopeMean, pch = 19, main = "Mean size vs ann SST", ylab = "Regression slope", xlab = "midpoint")
abline (h=0)

plot(IndRegr_gdd$midpoint, IndRegr_gdd$slopeMean, pch = 19, main = "Mean size vs GDD", ylab = "Regression slope", xlab = "midpoint")
abline (h=0)

plot(IndRegr_temp2yago$midpoint, IndRegr_temp2yago$slopeMean, pch = 19, main = "Mean size vs SST2 years ago", ylab = "Regression slope", xlab = "midpoint")
abline (h=0)

par(mfrow = c(1,1))
plot(IndRegr_annSST$slopeMean, IndRegr_temp2yago$slopeMean, pch = 19)
cor.test(IndRegr_annSST$slopeMean, IndRegr_temp2yago$slopeMean) ##0.94

plot(IndRegr_annSST$slopeMean, IndRegr_gdd$slopeMean, pch = 19)
cor.test(IndRegr_annSST$slopeMean, IndRegr_gdd$slopeMean) ## 0.99
